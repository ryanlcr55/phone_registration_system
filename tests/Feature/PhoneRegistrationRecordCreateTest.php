...
class PhoneRegistrationRecordCreateTest extends TestCase
{
    ...
    public function testPhoneRegistrationCreateApi()
    {
        ...
        $response = $this->post('/api/phoneRegistrationRecord/', [
            'from' => '0912122212',
            'text' => '場所代碼：' . $this->store->store_code . '\n本簡訊是簡訊實聯制發送，限防疫目的使用',
    ...
    public function testPhoneRegistrationCreateByJob()
    {
        $phoneNum = Str::random(10);
        $dataTime = Carbon::now();

        $job = new PhoneRegister($phoneNum, $this->store->store_code, $dataTime);
        $job->handle();

        $record = PhoneRegistrationRecord::query()
            ->where('phone_num', $phoneNum)
            ->where('store_code', $this->store->store_code)
